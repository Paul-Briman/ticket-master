...
const promotionsKey = `${PREFIX}promotions`
const promotionsSeededKey = `${PREFIX}promotions-seeded`
const homepageSectionsKey = `${PREFIX}homepage-sections`
...
export const db = {
  ...
  // ---------- Promotions ----------
  // Stored as a flat array at tm:promotions. Status (active /
  // scheduled / expired / disabled) is DERIVED at request time from
  // enabled + startsAt + endsAt + the current clock — no stored
  // status field, so expiry is automatic with no cron required.
  async listPromotions() {
    return (await rawGet(promotionsKey)) || []
  },

  async getPromotion(id) {
    const list = await this.listPromotions()
    return list.find((p) => p.id === id) || null
  },

  async savePromotion(promotion) {
    if (!promotion?.id) throw new Error('promotion.id required')
    const list = await this.listPromotions()
    const idx = list.findIndex((p) => p.id === promotion.id)
    if (idx >= 0) list[idx] = promotion
    else list.push(promotion)
    await rawSet(promotionsKey, list)
    return promotion
  },
  ...
  async deletePromotion(id) {
    ...
    if (next.length === list.length) return false
    await rawSet(promotionsKey, next)
    return true
  },

  // Seed marker — set to true the first time we lazy-seed the World
  // Cup Knockout Sale. Once set, even if the admin later deletes
  // every promotion the seeder will NOT recreate them. This avoids
  // the footgun where an admin's manual delete gets silently undone.
  async isPromotionsSeeded() {
    return !!(await rawGet(promotionsSeededKey))
  },

  async markPromotionsSeeded() {
    await rawSet(promotionsSeededKey, true)
  },

  // ---------- Homepage Sections ----------
  // Admin-controlled ordering, enabling, and per-section display
  // limits for the homepage lanes. Stored as an ordered array (not
  // a map) so admin drag-reorder writes are trivially serializable.
  // Null result → the routes layer seeds the default set and returns
  // that instead. Never used by anything other than the homepage.
  async getHomepageSections() {
    return await rawGet(homepageSectionsKey)
  },

  async saveHomepageSections(sections) {
    if (!Array.isArray(sections)) throw new Error('sections must be an array')
    await rawSet(homepageSectionsKey, sections)
    return sections
  },
}
