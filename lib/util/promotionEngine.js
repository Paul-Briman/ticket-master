...
export function applyPromotionsToEvent(event, promotions, now = Date.now()) {
  ...
  return {
    ...
    // know about promotions still shows the discounted price. The
    // surrounding UI (EventCard, etc.) prepends the word "From" — the
    // convention from normalize.js is that event.price is just the
    // dollar amount ("$80"), NOT "From $80". Prepending here caused
    // a "From From $80" rendering bug downstream.
    price: lowest !== null ? `$${lowest.toFixed(0)}` : event.price,
    promotion: {
      id: chosen.id,
      name: chosen.name,
...
export function normalizePromotionInput(input, existing = null) {
  ...
  // featured = "this is the campaign currently spotlighted on the
  // homepage banner". Only one promotion is featured at a time; the
  // routes layer enforces uniqueness by clearing the flag on every
  // other promo when one is marked featured.
...
/**
 * Pick the single promotion that should appear in the homepage
 * banner. Rules:
 *   1. Must be featured: true
 *   2. Must be currently ACTIVE (enabled + within startsAt..endsAt)
 * If multiple match (shouldn't happen because we enforce uniqueness
 * at the route layer, but defensive), the most recently updated wins.
 *
 * Returns null when no featured promo is currently running, which
 * the frontend reads as "don't show the banner."
 */
export function pickFeaturedPromotion(promotions, now = Date.now()) {
  if (!Array.isArray(promotions) || promotions.length === 0) return null
  const candidates = promotions.filter(
    (p) => p?.featured === true && derivePromotionStatus(p, now) === STATUS.ACTIVE,
  )
