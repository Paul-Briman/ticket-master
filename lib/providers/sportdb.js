...
async function safeJson(url) {
  ...
  if (!text || text.trim().startsWith('<')) {
    // SportsDB free tier returns HTML on rate-limit
...
export async function fetchEventById(id) {
  ...
  const r = await safeJson(`${BASE}/${apiKey()}/lookupevent.php?id=${encodeURIComponent(sdbId)}`)
  if (!r.ok) return null
  const ev = Array.isArray(r.json?.events) ? r.json.events[0] : null
  if (!ev) return null
  // Reverse-map league for orphan lookups
  const reverse = (s) => {
  ...
  return mapEvent(ev, reverse(ev.strLeague))
}
