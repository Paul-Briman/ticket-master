...
// Default pricing tiers derived from a base price. Admin can override
// any of these per event via /api/admin/events/:id.
export function defaultPricingTiers(seed, category, basePrice) {
  const standard = basePrice ?? synthPrice(seed, category)
  return {
    standard,
    premium: Math.round(standard * 1.5),
    vip: Math.round(standard * 2.4),
  }
}
...
export function normalizeEvent({
  ...
  pricing = null,      // { standard, premium, vip } — synthesized if absent
  badge = null,
  badgeType = null,
  provider,
  raw,
  ...
  // Parse base price for fallback synthesis. price string like "$120".
  const numericBase =
    price && typeof price === 'string'
      ? Number(price.replace(/[^0-9.]/g, '')) || null
      : null
  const tiers = pricing || defaultPricingTiers(id, category, numericBase)
