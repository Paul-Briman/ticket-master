...
router.get('/', async (req, res) => {
  try {
    ...
    // Snapshot every event so the unified /api/events/:id detail
    // endpoint can serve from DB without a re-fetch round-trip.
    try {
      await db.snapshotEvents(data.events)
    } catch (err) {
      console.warn('[concerts] snapshot failed:', err.message)
    }
