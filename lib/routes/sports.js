  ...
  try {
    ...
    if (requestedLeague) {
      ...
      try {
        ...
        console.warn(`[sports] snapshot failed for ${requestedLeague}:`, err.message)
      }
    ...
    try {
      ...
      console.warn('[sports] snapshot failed for aggregate:', err.message)
    }
    const overrides = await db.listEventOverrides()
    const merged = applyOverridesToList(data.events, overrides)
    res.setHeader('Cache-Control', 'public, max-age=300, s-maxage=300')
  ...
  try {
    ...
    if (live) {
      try {
        ...
        console.warn('[sports/:id] snapshot write failed', { id, error: err.message })
      }
    }
    ...
    const merged = override ? applyEventOverride(base, override) : base
