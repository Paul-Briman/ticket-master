...
router.get('/', async (req, res) => {
  try {
    ...
    if (requestedLeague) {
      ...
      const merged = filterVisibleEvents([...adminMerged, ...liveMerged])
  ...
  try {
    ...
    if (String(id).startsWith('adm-')) {
      ...
      const merged = override
        ...
        : adminEvent
      const event = { ...merged, expired: !isEventVisible(merged) }
    ...
    const merged = { ...overridden, expired: !isEventVisible(overridden) }
