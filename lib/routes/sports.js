...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (String(id).startsWith('sdb-')) {
      const cacheKey = `sports:event:sdb:${id}`
      const { data, source } = await swr(
        cacheKey,
        () => sportdb.fetchEventById(id),
        { ttlSeconds: 900, staleSeconds: 3600 },
      )
      if (!data) return res.status(404).json({ error: 'Event not found' })
      res.setHeader('Cache-Control', 'public, max-age=300, s-maxage=300')
      return res.status(200).json({ event: data, cacheSource: source })
    }

    return res.status(404).json({
      error: 'Event not found',
      message: 'Only sdb-prefixed lookups are supported right now.',
    })
  } catch (err) {
