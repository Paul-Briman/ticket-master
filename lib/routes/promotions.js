...
// Enforce single-featured invariant: when we're about to save a
// promotion marked featured=true, clear featured on every OTHER
// promotion first. Idempotent + safe to call even when the saved
// promo is NOT featured (in which case it's a no-op).
async function clearOtherFeatured(savedId) {
  const list = await db.listPromotions()
  for (const p of list) {
    if (p.id !== savedId && p.featured === true) {
      await db.savePromotion({ ...p, featured: false, updatedAt: new Date().toISOString() })
    }
  }
}
...
router.get('/promotions', async (_req, res) => {
  try {
    ...
    const featured = pickFeaturedPromotion(list, now)
    return res.status(200).json({
      promotions: decorated,
      count: decorated.length,
      featured: featured ? decorate(featured, now) : null,
    })
  } catch (err) {
...
// Dedicated thin endpoint so the homepage can fetch ONLY the
// featured promo (faster — avoids deserializing every promo on
// every homepage hit). Returns 204 No Content when there is no
// currently-active featured promo so the client can short-circuit
// without rendering the banner shell.
router.get('/promotions/featured', async (_req, res) => {
  try {
    await ensurePromotionsSeeded()
    const list = await db.listPromotions()
    const now = Date.now()
    const featured = pickFeaturedPromotion(list, now)
    // Short cache (30s) — matches the list-endpoint TTL so the
    // homepage banner and the surrounding event grid update
    // together after an admin flips a "Featured" flag.
    res.setHeader('Cache-Control', 'public, max-age=30, s-maxage=30')
    if (!featured) return res.status(204).end()
    return res.status(200).json({ promotion: decorate(featured, now) })
  } catch (err) {
    return handleError(res, err, 'promotions-featured')
  }
})
...
admin.post('/', async (req, res) => {
  try {
    ...
    if (result.promotion.featured) await clearOtherFeatured(result.promotion.id)
...
admin.post('/:id/clone', async (req, res) => {
  try {
    ...
    const result = normalizePromotionInput({
      ...
      featured: false, // and never automatically inherit "featured" — only one campaign features at a time
    })
    if (!result.ok) return res.status(400).json({ error: result.error })
    await db.savePromotion(result.promotion)
    return res.status(201).json({ ok: true, promotion: decorate(result.promotion) })
