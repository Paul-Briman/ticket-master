...
router.get('/', async (req, res) => {
  try {
    ...
    // Admin-created events: if for some reason an override was layered
    // on top of one (rare — the form patches the record itself), apply
    // it. Mark every admin event so the UI can render distinct actions.
    ...
    const merged = [...adminMerged, ...liveMerged]
...
router.patch('/:id', async (req, res) => {
  try {
    ...
    // upstream record stays untouched.
    const allowed = [
      'title',
      'image',
      'venue',
