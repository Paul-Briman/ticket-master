...
function normalizeAdminEventInput(body, { partial = false } = {}) {
  ...
  // Homepage editorial fields — used by Home.jsx to pin events to a
  // section and control their display order.
  if (
    body.featuredSection === null ||
    typeof body.featuredSection === 'string'
  ) {
    out.featuredSection = body.featuredSection || null
  }
  if (body.featuredOrder === null || body.featuredOrder === '') {
    out.featuredOrder = null
  } else if (body.featuredOrder !== undefined) {
    const n = Number(body.featuredOrder)
    if (Number.isFinite(n)) out.featuredOrder = n
  }
...
router.patch('/:id', async (req, res) => {
  try {
    ...
    // Homepage editorial fields. Booleans + numbers need different
    // guards from the whitelist above (that loop rejects unless
    // `!== ''`, which incorrectly filters false/0). Explicit here.
    if (typeof body.featured === 'boolean') patch.featured = body.featured
    if (
      body.featuredSection === null ||
      typeof body.featuredSection === 'string'
    ) {
      patch.featuredSection = body.featuredSection || null
    }
    if (body.featuredOrder === null || body.featuredOrder === '') {
      patch.featuredOrder = null
    } else if (body.featuredOrder !== undefined) {
      const n = Number(body.featuredOrder)
      if (Number.isFinite(n)) patch.featuredOrder = n
    }
    if (body.pricing && typeof body.pricing === 'object') {
      const s = cleanTier(body.pricing.standard)
