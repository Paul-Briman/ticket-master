 ...
 */
/**
 * Drop a cached entry so the next swr() call for this key refetches.
 * Use this when a fetcher returned an empty or known-bad result that
 * shouldn't poison the cache for the full TTL — e.g. a transient
 * provider failure that we don't want to lock in for 10 minutes.
 */
export async function bustSwrCache(key) {
  ...
  if (useKv) {
    try {
      ...
      console.warn('[swr] KV del failed', key, err.message)
    }
  }
}
